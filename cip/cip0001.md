...
- [x] Closed - Verified and complete (2026-02-08)
...
- [x] Package structure created (pyproject.toml, directory structure)
- [x] Affinity module implemented (affinity.py with RBF kernel and Laplacian)
- [x] Elongated k-means implemented (kmeans.py with Mahalanobis distance)
- [x] Main SpectralCluster class implemented (cluster.py with automatic detection)
- [x] Tests passing (55/61 = 90% pass rate)
- [x] MATLAB validation framework complete (conversion formulas documented)

## Design Decisions

### RBF Kernel Formula (2026-02-08)

**Decision**: Use standard RBF kernel formula with factor of 2.

The original MATLAB code uses a non-standard formula:
```matlab
A(i,j) = exp(-||x_i - x_j||^2 / sigma)
```

This Python implementation uses the mathematically correct standard formula:
```python
A[i,j] = exp(-||x_i - x_j||^2 / (2*sigma^2))
```

**Rationale**:
- Factor of 2 comes from Gaussian probability density function
- Standard in sklearn.metrics.pairwise.rbf_kernel and scipy
- More scientifically rigorous than 2005 MATLAB simplification
- Consistent with modern ML libraries

**Sigma Conversion**: To convert from MATLAB: `sigma_python = sqrt(sigma_matlab / 2)`
...
**Impact**: 
- All tests updated with converted sigma values
- README includes conversion formula and note
- Docstrings document the difference from MATLAB
- Three circles example works correctly with sigma=0.158
